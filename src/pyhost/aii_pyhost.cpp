...
def _run(path):
    try:
        ...
        aii.status("%s: %s" % (path.replace("\\", "/").rsplit("/", 1)[-1], last), False)
...
try:
    while not aii.should_quit() and any(t.is_alive() for t in _threads):
        time.sleep(0.05)
...
bool aiiPyHostRegister(aii::AppBus* bus, const char* const* scriptPaths, int scriptCount) {
  if (!bus || !scriptPaths || scriptCount <= 0) return false;
  if (g_host.bus) return false;
  g_host.scripts.assign(scriptPaths, scriptPaths + scriptCount);
